...
class MinimalPublisher(Node):
    def __init__(self):
        ...
        self.controller1_publisher_ = self.create_publisher(
            Controller, "controller1", 10
        )
        self.controller2_publisher_ = self.create_publisher(
    ...
    def hmd_publish(self, x, y, z, yaw, pitch, roll):
        msg = Hmd()
        msg.x = x
        msg.y = y
        msg.z = z
        ...
        msg.pitch = pitch
        msg.roll = roll
        self.hmd_publisher_.publish(msg)

    def controller1_publish(
        self,
        x,
        y,
        z,
        ...
        msg.menu = menu
        msg.trigger = trigger
        msg.trackpad_pressed = trackpad_pressed
        msg.trackpad_touched = trackpad_touched
        msg.trackpad_x = trackpad_x
        msg.trackpad_y = trackpad_y
        self.controller1_publisher_.publish(msg)

    def controller2_publish(
        self,
        x,
        y,
        z,
        ...
        msg.trackpad_x = trackpad_x
        msg.trackpad_y = trackpad_y
        self.controller2_publisher_.publish(msg)


def hmd_run(minimal_publisher, device):
    while True:
...
def controller1_run(minimal_publisher, device):
    while True:
        pose = device.get_pose_euler()
        ...
        if not pose:
            pose = previous_pose
        # Publish
        minimal_publisher.controller1_publish(
            float(pose[0]),
            float(pose[1]),
            float(pose[2]),
            float(pose[3]),
            ...
            float(controller_inputs["trigger"]),
            int(controller_inputs["trackpad_pressed"]),
            int(controller_inputs["trackpad_touched"]),
            float(controller_inputs["trackpad_x"]),
            float(controller_inputs["trackpad_y"]),
        )
        previous_pose = pose


def controller2_run(minimal_publisher, device):
    while True:
        pose = device.get_pose_euler()
        controller_inputs = device.get_controller_inputs()
        if not pose:
            pose = previous_pose
        # Publish
        minimal_publisher.controller2_publish(
            float(pose[0]),
            float(pose[1]),
            float(pose[2]),
            float(pose[3]),
